...
radioButtons(
  "assign",
  label = HTML("<b>Do you want to assign unmatched species to a threat listing?</b>"),
  choices = list("No" = "No", 
                 "Yes" = "Yes"),
  selected = "No"
)
...
This tab allows on-the-fly re-allocation of a species to a particular user described threat listing. This will result in an updated *summary* table.
...
renderUI({
  selectInput(
    ...
    choices = unique(my.incompletes()$species)
  )
})
...
renderUI({
  selectInput("assign.cat",
              label = "New category",
              choices = unique(nztcs$Category))
})

renderUI({
  selectInput("assign.statx",
              label = "New status",
              choices = unique(nztcs$Status))
})
...
nztcs_summary <- reactive({
  ...
  colnames(interim) <- c("Category", "Status", "Species")
...
new.table <- reactive({
 ...
 my.temp.sum
  ...
  #unmatched <- nztcs_summary %>% filter(category == "No match in NZTCS")
...
my.new <- bind_rows(nztcs_summary.renamed, no.matches, my.temp.sum())

refer <- which(my.new$Category== "No match in NZTCS")

my.new[refer, 3] <- my.new[refer, 3] - sum(my.temp.sum()$Species)
