...
non.mig <- reactive({
  st_read("Non-migratory_Freshwater_Fish_Distribution/Non_migratory_Freshwater_Fish_Distribution.shp", quiet = TRUE) %>% 
    filter(Species == "Galaxias anomalus") %>% st_transform(crs = 4326)
...
### Possible discrepancies (species only)
...
renderLeaflet({
    ...
    # get nearest
  nearest <- st_nearest_feature(my.taxon(), non.mig())
  
  # get distance
  dist <-
    st_distance(my.taxon(), non.mig()[nearest, ], by_element = TRUE)
  ...
  for (i in 1:nrow(my.taxon())) {
    my.feature[[i]] <-
      st_nearest_points(my.taxon()[i,], non.mig()[nearest[i],]) %>%
      st_sf()
  }
