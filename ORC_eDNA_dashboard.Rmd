...
```{r}
# my.polygon conditional selections
# conditional on submit

polygon.choices <- reactive({
                         
  if (input$spatialfile == "polygon") {
    polyfish <- my.shape()
    st_geometry(polyfish) <- NULL
    polynames <- select_if(polyfish, is.character) %>% names()
    polygon.choices <- sort(polynames)
    polygon.choices
    
  } else {
    polygon.choices <- NULL
    polygon.choices
  }
})

# species choices for my.polygon
renderUI({
  conditionalPanel(condition = "input.spatialfile == 'polygon'",
                   
                   selectInput("polygon.var",
                               label = "Pop up information from:",
                               choices = polygon.choices()))
...
my.map <- eventReactive(input$submit,
  ...
  if (input$spatialfile == "nmfish") {
    ...
    my.polygon <- my.shape()
    my.polygon$present <- lengths(st_intersects(my.polygon, my.taxon())) > 0
    my.polygon$present <- ifelse(my.polygon$present == TRUE, "red", "black")

    my.popup.col <- which(colnames(my.polygon)== input$polygon.var)
    ...
    loc.map() %>%
      addPolygons(
        ...
        popup = my.polygon[[input$polygon.var]]
      )
...
aberrant <- reactive({
 ...
 if (input$spatialfile == "nmfish") {
   ...
   non.mig.sp <- my.shape() %>% 
    ...
    non.mig.sp.flat <- non.mig.sp  %>% st_transform(crs = 2193)
    
    # get distance
    dist.m <-
      st_distance(my.taxon.flat , non.mig.sp.flat[nearest,], 
                  by_element = TRUE)
...
renderPrint({
    ...
    if (identical(A, character(0)) == TRUE) {
      ...
      your_polylines = A 
          ...
          B <- list.files(paste0(getwd(), "/Your Species Polygons"))
    ...
    C <- list.files(paste0(getwd(), "/Your Non-Migratory Freshwater Fish Distribution"))
    ...
    list("Polyline check" = your_polylines, 
