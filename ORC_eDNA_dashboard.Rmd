...
renderDataTable({
  ...
  my.status() %>% group_by(species, Category, Status) %>% tally()
    
  } else {
    
   my.category() %>% group_by(species, Category, Status)  %>% tally()
...
renderDataTable({
  ...
  my.incompletes <- my.nztcs()[!complete.cases(my.nztcs()$Category), ] %>%
    as.data.frame()
...
renderDataTable({
  ...
  interim <-
    my.nztcs() %>% group_by(species, Category, Status)  %>% tally()
  interim <- interim %>% group_by(Category, Status) %>% tally()
  ...
  interim$Status <-
    ifelse(is.na(interim$Status), "No match in NZTCS", interim$Category)
