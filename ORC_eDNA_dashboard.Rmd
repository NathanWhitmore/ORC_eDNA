...
non.mig <- reactive({
  st_read("Non_migratory_Freshwater_Fish_Distribution.shp", quiet = TRUE) %>% 
    filter(Species == my.taxon())
...
renderLeaflet({
  ...
  if (is.null(input$taxon.dis) == FALSE) {
    
    # get nearest
    nearest <- st_nearest_feature(my.taxon(), taxon_dis())
    
    # get distance
    dist <-
      st_distance(my.taxon(), taxon_dis()[nearest, ], by_element = TRUE)
    
    # make line between points and nearest known location via loop
    my.feature <- list()
    
    for (i in 1:nrow(my.taxon())) {
      my.feature[[i]] <- st_nearest_points(my.taxon()[i, ], taxon_dis()[nearest[i], ]) %>%
        st_sf()
    }
    
    # nearest jump
    nearest_jump <- bind_rows(my.feature)
    nearest_jump <- nearest_jump %>% st_transform(crs = 4326)
    
    # my.taxon()$distance <- paste(round(as.numeric(dist) / 1000, 2), "km")
