...
eDNA <- reactive ({
  if (input$origin == "wilderlab") {
    ...
    # read taxa (stored locally) for classification information
    
    my.taxa <- read.csv("Data/taxa.csv")
    
    my.full <- left_join(my.full, my.taxa, by = "TaxID")
    my.full$Sequence <- NULL
    my.full$Target <- NULL
    my.full$MaoriName <- NULL
    
    
    # get lineages
    tdb <- my.full[, c(3, 8, 9, 10)]
    colnames(tdb) <- c("taxID", "parent_taxID", "rank", "name")
...
* Choose the species you want to reallocate from the list below.
...
nztcs_summary <- reactive({
  ...
  interim <- interim %>% arrange(Category, Status, Species)
  interim
...
on_the_fly <- reactive({
  ...
  my.new <- my.new %>% arrange(Category, Status, Species)
  my.new
