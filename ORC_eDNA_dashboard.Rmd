...
renderDataTable({
  ...
  my.status() %>% group_by(species, Status) %>% tally()
    
  } else {
    
   my.category() %>% group_by(species, Category)  %>% tally()
...
renderDataTable({
  ...
  interim <- my.nztcs() %>% group_by(species, Category, Status)  %>% tally()
...
renderPlot({
  ...
  my.nzcts.sum <- my.nztcs() %>%
    group_by(Category, Status) %>%
    summarise(count = n())
  
  ggplot()+
    geom_point(data = my.nzcts.sum, aes(x = Status, y = count , colour = Category),
               size = 10)
