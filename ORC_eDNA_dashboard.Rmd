...
tryCatch({
  ...
  st_read("Your Non-Migratory Freshwater Fish Distribution/Non_migratory_Freshwater_Fish_Distribution.shp", quiet = TRUE) %>% 
    st_transform(crs = 4326)
  
},  error = function(e){
  message("It appears your fish distribution files are not correctly installed")
...
renderPrint({
  ...
  if (input$spatial.file == "nmfish") {
    A <- list.files(paste0(getwd(), "/Your Non-Migratory Freshwater Fish Distribution"))
    
    if (identical(A, character(0)) == TRUE) {
      showModal(modalDialog("No files are present. Please copy file(s) into: 'Your Non-Migratory Freshwater Fish Distribution'"))
      
    } else {
      print(A, row.name = FALSE)
    }
    
    
    
  } else if (input$spatial.file == "polyline") {
    A <- list.files(paste0(getwd(), "/Your Polylines"))
    
    if (identical(A, character(0)) == TRUE) {
      showModal(modalDialog("No files are present. Please copy file(s) into: 'Your Polylines'"))
    ...
    A <- list.files(paste0(getwd(), "/Your Polygons"))
    
    if (identical(A, character(0)) == TRUE) {
      showModal(modalDialog("No files are present. Please copy file(s) into: 'Your Polygons'"))
