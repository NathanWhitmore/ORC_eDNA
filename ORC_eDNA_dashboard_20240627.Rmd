...
eDNA <- reactive ({
  if (input$origin == "wilderlab") {
    ...
    # remove invalidate sample points
    condition_A <- which(is.na(samples$Latitude))
    condition_B <-which(is.na(samples$Longitude))
    condition_C <-which(samples$Longitude >180 | samples$Longitude < -180)
    condition_D <-which(samples$Latitude >90 | samples$Latitude < -90)

    removals <- unique(c(condition_A, condition_B, condition_C, condition_D ))
    
    # if there are no removals return samples
    samples <- if (identical(removals, integer(0))){samples} else {samples[-removals,]}
    ...
    # faulty samples for 
    ##faulty_samples_local <<- biodiversity.meta.data[removals,]
    ...
    # spatial check algorithm
    # ensure lat long are numeric
    eDNA$latitude <-  as.numeric(eDNA$latitude)
    eDNA$longitude <- as.numeric(eDNA$longitude)
        
     # remove invalidate sample points
    condition_A <- which(is.na(eDNA$latitude))
    condition_B <- which(is.na(eDNA$longitude))
    condition_C <- which(eDNA$longitude >180 | eDNA$longitude < -180)
    condition_D <- which(eDNA$latitude >90 | biodiversity.meta.data$latitude < -90)

    removals <- unique(c(condition_A, condition_B, condition_C, condition_D ))
    
    # if there are no removals return samples
    eDNA <- if (identical(removals, integer(0))){eDNA} else {eDNA[-removals,]}
...
renderDataTable({
  ...
  # record indexes of nonsensical spatial records
  condition_A <- which(is.na(samples$Latitude))
  condition_B <- which(is.na(samples$Longitude))
  condition_C <-
    which(samples$Longitude > 180 | samples$Longitude < -180)
  condition_D <- which(samples$Latitude > 90 | samples$Latitude < -90)
  
  removals <-
    unique(c(condition_A, condition_B, condition_C, condition_D))
  
  # if there are no removals return samples
samples <- if (identical(removals, integer(0))){samples} else {samples[-removals,]}

# if there are no removals make faulty samples empty
faulty <- if (identical(removals, integer(0))){
  samples[FALSE, ]} else 
  {samples[removals,]}
  ...
  datatable(faulty,
    ...
    # record indexes of nonsensical spatial records
    condition_A <- which(is.na(biodiversity.meta.data$Latitude))
    condition_B <- which(is.na(biodiversity.meta.data$Longitude))
    
    biodiversity.meta.data$Latitude <- as.numeric(as.character(biodiversity.meta.data$Latitude))
        biodiversity.meta.data$Longitude <- as.numeric(as.character(biodiversity.meta.data$Longitude))
    
    condition_C <- which(biodiversity.meta.data$Longitude > 180 | biodiversity.meta.data$Longitude < -180)
    condition_D <- which(biodiversity.meta.data$Latitude > 90 |    biodiversity.meta.data$Latitude < -90)
...
To render the video locally you may have to click *Open in Browser* on the top lefthand side of this window. Alternatively, please click the links below:
 * [sample location](https://www.loom.com/share/14acb4efd50e4633a67e9b8c990f5081?sid=c238d4a3-a98e-4187-85b2-046efa65119e)
 * [taxonomy](https://www.loom.com/share/849c039eaaff459d86e4797880518c0b?sid=fa804f18-6d6d-426b-807c-eb685f9e32dc)
 * [threat status](https://www.loom.com/share/7e9928c14f844618ae31483db9035a50?sid=1ba3c474-9382-42bb-a1db-e0b619f93d6d)
 * [cluster](https://www.loom.com/share/20e805bb0382489cb5d5677f40e768cb?sid=fff17df5-421c-42e1-8d7a-e10e60293454)
 * [knowledge](https://www.loom.com/share/37387f5d94c44cbf87e12bb7739c33db?sid=c4ef3ca6-ae2e-4323-af71-01d2f719adc9)
...
The code for this dashboard was written by Nathan Whitmore is freely available for reuse and modification via an [MIT license](https://choosealicense.com/licenses/mit/). The source code can be found under the under the '</> Source Code' tab or via [GitHub](https://github.com/NathanWhitmore/ORC_eDNA).  Dashboard creation was funded by the Otago Regional Council. Special thanks go to Scott Jarvie and Ciaran Campbell for instigating this project.


Row {.tabset .tabset-fade}
-------------------------------------

### Video: sample location
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/14acb4efd50e4633a67e9b8c990f5081?sid=8e317955-ea81-474c-b284-b38053005ec9" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>

### Video: taxonomy
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/849c039eaaff459d86e4797880518c0b?sid=754ee04c-d671-48b8-95f3-0a53b5229a65" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>

### Video: threat status
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/7e9928c14f844618ae31483db9035a50?sid=3c3f90a7-7296-4209-a3ac-9edafe341d1e" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>

### Video: spatial
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/14ba92a616c74c88bb03258ffd94e926?sid=389fdff4-a104-4cbd-91d4-4de4371fbb9c" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>

### Video: cluster
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/20e805bb0382489cb5d5677f40e768cb?sid=9005aeb3-8291-4afd-82b9-a81004465552" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>

### Video: knowledge
<div style="position: relative; padding-bottom: 56.25%; height: 0;"><iframe src="https://www.loom.com/embed/37387f5d94c44cbf87e12bb7739c33db?sid=51f50f2f-952f-46b8-99a1-5b292342be6f" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 80%; height: 80%;"></iframe></div>
